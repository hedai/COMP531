...
const configFacebookAuth = {
  ...
  callbackURL: 'https://dhbookhw7.herokuapp.com/auth/facebook/callback',
    ...
    if(!sid){
      User.findOne({username: username}).exec(function(err, user) {
        if(!user || user.length === 0){
          ...
          const profileObj = new Profile({username: username, headline: "login by facebook", following:[], email: null, zipcode: null, dob: new Date(1999,09,09).getTime(), avatar: "https://www.facebook.com/images/fb_icon_325x325.png"})
...
function isLoggedIn(req, res, next){
  ...
  if (req.isAuthenticated()) {
    console.log('call isAuthenticated', req.user.username)
...
const successLogin = (req,res) => {
  console.log('call successLogin', req.body)
  res.redirect(originHostUrl)
}

const errorLogin = (err,req,res,next) => {
  console.log('call errorLogin', req.body)
    if(err) {
        res.status(400);
...
module.exports = app => {
  ...
  app.use('/auth/facebook/callback', passport.authenticate('facebook', {failureRedirect:'/login/facebook'}), successLogin, errorLogin)
